...
public class GoalManager
{
  ...
  public void RecordEvent()
  {
    ...
    if (goalIndex >= 0 && goalIndex < _goals.Count)
    {
      ...
      {
        Console.WriteLine($"Oops! You lost {Math.Abs(pointsEarned)} points for doing a bad habit.");
      }

      // Leveling up logic (1 level per 1000 points)
  ...
  public void LoadGoals()
  {
    ...
    if (File.Exists(fileName))
    {
      ...
      _level = (_score / 1000) + 1; // Recalculate level based on loaded score
      _goals.Clear();
      ...
      for (int i = 1; i < lines.Length; i++)
      {
        ...
        if (goalType == "SimpleGoal")
        {
          ...
          {
            sg.RecordEvent(); // Mark as complete if saved as true
          }
          _goals.Add(sg);
        }
        ...
        else if (goalType == "ChecklistGoal")
        {
          ...
          int currentAmount = int.Parse(details[5]);
          for (int j = 0; j < currentAmount; j++)
          {
            cg.RecordEvent(); // Fast-forward progress
          }
          _goals.Add(cg);
        }
